...
export default function TemplateEditor({ visible, editingTemplate, onSave, onClose }: Props) {
  ...
  const handleZoom = useCallback((delta: number) => {
    setZoom(z => Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, z + delta * ZOOM_STEP)));
  }, []);
  ...
  const drawGrid = useCallback(() => {
    ...
    ctx.textAlign = 'center';
    for (let c = 0; c < w; c++) {
      ctx.fillText(String(c), TE_PAD + c * TE_SPACING + TE_SPACING / 2, TE_PAD - 6);
    }
    ctx.textAlign = 'right';
    for (let r = 0; r < h; r++) {
  ...
  const holeFromMouse = useCallback((e: React.MouseEvent<HTMLCanvasElement>): [number, number] | null => {
    const canvas = canvasRef.current;
  ...
  const handleSubmit = useCallback((_e: React.FormEvent, helpers: { close: () => void }) => {
    if (responseStatus) return;
    if (!name.trim()) {
      setResponseStatus({ status: 'error', text: '部品名を入力してください' });
      return;
    }
    if (pins.length === 0) {
      setResponseStatus({ status: 'error', text: '少なくとも1つのピンを配置してください' });
      return;
    }
          ...
          <Stack gap={0.75}>
            <FormControl label="部品名">
              <Input value={name} width="100%"
                onChange={e => { setName(e.target.value); setResponseStatus(undefined); }} />
            </FormControl>
            <Cluster gap={0.5}>
              <FormControl label="幅 (列)">
                <Input type="number" value={w} min={1} max={30} width="100%"
                  onChange={e => handleResize(Number(e.target.value), h)} />
              </FormControl>
              <FormControl label="高さ (行)">
                <Input type="number" value={h} min={1} max={30} width="100%"
            ...
            <FormControl label="色">
              <Cluster gap={0.5} align="center">
                ...
                <Text size="S" color="TEXT_GREY">{color}</Text>
              </Cluster>
            </FormControl>
            ...
            <div>
              ...
              <div className={styles.pinList}>
                {pins.length === 0 ? (
                  <Text size="S" color="TEXT_GREY">ピンなし</Text>
                ) : (
                  pins.map((pin, i) => (
                    <div key={i} className={styles.pinItem}>
                      <span className={styles.pinNum}>{i + 1}</span>
                      <Text size="S" color="TEXT_GREY" className={styles.pinPos}>({pin.r},{pin.c})</Text>
                      <Input
                        value={pin.label}
                        width="100%"
                        ...
                        onChange={e => {
                          ...
                          setPins(prev => prev.map((p, j) => j === i ? { ...p, label: val } : p));
                        }}
